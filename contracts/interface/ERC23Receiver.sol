...
contract ERC23Receiver {
  ...
  From: the origin of the tokens sent
  ...
  From and sender shouldn't be trusted unless the token contract is trusted.
  If sender == tx.origin, it is safe to trust it regardless of the token.
  */

  function tokenFallback(address _sender, address _from, uint _value, bytes _data) returns (bool ok);
}
